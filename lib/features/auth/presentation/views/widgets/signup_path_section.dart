...
class SignupPathSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isStudent = userProvider.isStudent;
    final lang = S.of(context);
    return SafeArea(
      child: Scaffold(
        appBar: CustomAppBar(
            ...
            child: Column(
              children: [
                ...
                StudentOrNotButton(
                  ...
                  selectNonStudent: isStudent==false,
                  onSelectStudent:()=> userProvider.setIsStudent(value: true),
                  onSelectNotStudent:()=> userProvider.setIsStudent(value: false),
                ),
                SizedBox(height: 300),
              ],
            ),
        ...
        bottomNavigationBar:
            (isStudent != null)
                ? Padding(
                  ...
                  child: NextButton(onPressed: ()=> handleNext(context,userProvider.isStudent)),
                )
                : null,
      ),
    );
  ...
  void handleNext(BuildContext context, bool? isStudent) {
    if(isStudent == true){
   Navigator.pushNamed(context, SignupCollageStageSection.name);
